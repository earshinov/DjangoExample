...
class Field(Model):
  ...
  # Казалось бы, условие неизменности таблицы назначения, fieldType и
  # bookmarkName после создания записи можно было обеспечить и на уровне
  # модели, подменив в конструкторе после вызова базового конструктора
  # соответствующие поля экземпляра (.target, .fieldType, .bookmarkName)
  # и .id свойствами (property) и выкидывая в свойствах исключение в
  # случае, когда значение пытаются поменять.  К сожалению, это не
  # работает, потому что Django устанавливает значения свойств с
  # использованием setattr (см. метод Field.save_form_data())
